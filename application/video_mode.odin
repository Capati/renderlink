...
get_video_modes :: proc(allocator := context.allocator) -> []Video_Mode {
    return #force_inline _get_video_modes(allocator)
}

get_video_mode :: proc() -> (mode: Video_Mode) {
    return #force_inline _get_video_mode()
}

video_mode_is_valid :: proc(mode: Video_Mode) -> bool {
    runtime.DEFAULT_TEMP_ALLOCATOR_TEMP_GUARD()
    modes := _get_video_modes(context.temp_allocator)
    for &curr_mode in modes {
        if video_mode_equals(curr_mode, mode) do return true
    }
    return false
}
