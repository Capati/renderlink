...
@(require_results)
_window_get_size :: proc(window: Window) -> Vec2u {
    impl := _window_get_impl(window)
...
_window_cleanup_callbacks :: proc(window: Window) {
    ...
    glfw.SetWindowCloseCallback(impl.handle, nil)
    glfw.SetFramebufferSizeCallback(impl.handle, nil)
    glfw.SetKeyCallback(impl.handle, nil)
    glfw.SetCursorPosCallback(impl.handle, nil)
    glfw.SetMouseButtonCallback(impl.handle, nil)
    glfw.SetScrollCallback(impl.handle, nil)
    glfw.SetWindowIconifyCallback(impl.handle, nil)
    glfw.SetWindowFocusCallback(impl.handle, nil)
}

@(private = "file")
_window_close_callback :: proc "c" (handle: glfw.WindowHandle) {
    impl := _window_get_user_pointer(handle)
    dispatch_event(impl.app, Quit_Event{})
}
...
_window_key_callback :: proc "c" (handle: glfw.WindowHandle, key, scancode, action, mods: i32) {
    ...
    event := Key_Event {
        key      = _to_key(key),
        scancode = _to_scancode(scancode),
...
_window_cursor_position_callback :: proc "c" (handle: glfw.WindowHandle, xpos, ypos: f64) {
    ...
    dispatch_event(impl.app,
        Mouse_Moved_Event{pos = {f32(xpos), f32(ypos)}, button = button, action = action},
    )
}
...
_window_mouse_button_callback :: proc "c" (handle: glfw.WindowHandle, button, action, mods: i32) {
    ...
    if action == glfw.PRESS {
        dispatch_event(impl.app, Mouse_Button_Pressed_Event(event))
    } else if action == glfw.RELEASE {
        dispatch_event(impl.app, Mouse_Button_Released_Event(event))
    }
}
...
_window_scroll_callback :: proc "c" (handle: glfw.WindowHandle, xoffset, yoffset: f64) {
    ...
    dispatch_event(impl.app, Mouse_Wheel_Event{f32(xoffset), f32(yoffset)})
}
...
_window_minimized_callback :: proc "c" (handle: glfw.WindowHandle, iconified: i32) {
    ...
    dispatch_event(impl.app, Minimized_Event{impl.is_minimized})
}
...
_window_focus_callback :: proc "c" (handle: glfw.WindowHandle, focused: i32) {
    ...
    dispatch_event(impl.app, Restored_Event{bool(focused)})
}
